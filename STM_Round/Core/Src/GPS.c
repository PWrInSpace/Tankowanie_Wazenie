...
void	GPS_Process(void)
{
	if((uwTick - GPS.LastTime>50) && (GPS.rxIndex>0))
	{
		...
		if(str!=NULL)
		{
			...
			sscanf(str,
					"$GNGGA,%02d%02d%02d.%03d,%f,%c,%f,%c,%01d,%02d,%f,%f,%c,%hd,%s,*%2s\r\n",
					(int*) &GPS.GPGGA.UTC_Hour, (int*) &GPS.GPGGA.UTC_Min, (int*) &GPS.GPGGA.UTC_Sec,
					(int*) &GPS.GPGGA.UTC_MicroSec, &GPS.GPGGA.Latitude,
					&GPS.GPGGA.NS_Indicator, &GPS.GPGGA.Longitude,
					&GPS.GPGGA.EW_Indicator, (int*) &GPS.GPGGA.PositionFixIndicator,
					(int*) &GPS.GPGGA.SatellitesUsed, &GPS.GPGGA.HDOP,
					&GPS.GPGGA.MSL_Altitude, &GPS.GPGGA.MSL_Units,
					&GPS.GPGGA.AgeofDiffCorr, GPS.GPGGA.DiffRefStationID,
					GPS.GPGGA.CheckSum);

...
Commands::Commands() {
	fill = new Motor(FILL_OPEN_GPIO_Port, FILL_OPEN_Pin, FILL_CLOSE_GPIO_Port,
	...
	depr = new Motor(DEPR_OPEN_GPIO_Port, DEPR_OPEN_Pin, DEPR_CLOSE_GPIO_Port,
...
//	quickrel = new Motor(QD_OPEN_GPIO_Port, QD_OPEN_Pin, QD_CLOSE_GPIO_Port,
//			QD_CLOSE_Pin,
//			QD_EN_GPIO_Port, QD_EN_Pin, QD_OPEN_LIMIT_SW_GPIO_Port,
//			QD_OPEN_LIMIT_SW_Pin, QD_CLOSE_LIMIT_SW_GPIO_Port,
//			QD_CLOSE_LIMIT_SW_Pin);
quickrel = nullptr;
}
...
void Commands::doCommand(char buff[]) {
	...
	case '1':
		if (stringCompare(buff, "TEST_MOTOR", strlen("TEST_MOTOR"))) {
			HAL_UART_Transmit(huart, (uint8_t*) "calibrating the valve \n",
			...
			HAL_UART_Transmit(huart, (uint8_t*) "^ wrong command received \n",

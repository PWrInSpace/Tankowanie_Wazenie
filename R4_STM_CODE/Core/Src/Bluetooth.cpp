...
void Bluetooth_module::resolveCommand() {

	HAL_UART_Transmit(this->huart, (uint8_t*) "in/n", strlen("in/n"),
			500);
	HAL_UART_Transmit(this->huart , (uint8_t*) buff, strlen(buff), 500);
	...
	HAL_UART_Transmit(this->huart, (uint8_t*) "exit /n", strlen("exit /n"),
			500);
}
...
bool Bluetooth_module::stringCompare(char array1[], char array2[], uint16_t lght) {
	uint8_t var = 0;
...
void Bluetooth_module::doCommand(Motor *Mot) {

	if (stringCompare(buff,  "TEST_MOTOR", strlen("TEST_MOTOR"))) {
		HAL_UART_Transmit(this->huart, (uint8_t*) "calibrating the valve \n",
				strlen("calibrating the valve \n"), 500);
		HAL_UART_Transmit(this->huart, (uint8_t*) "Done... \n",
				strlen("Done... \n"), 500);
		Mot->test_open_close();
		///////////////////////////
	} else if (stringCompare(buff, "OPEN", strlen("OPEN"))) {
		HAL_UART_Transmit(this->huart, (uint8_t*) "Opening \n",
				strlen("Opening \n"), 500);
		Mot->open(5);
		//////////////////
	} else if (stringCompare(buff, "CLOSE", strlen("CLOSE"))) {
		HAL_UART_Transmit(this->huart, (uint8_t*) "Closing \n",
				strlen("Closing \n"), 500);
		Mot->close(5);
		//////////////////////////
	} else if (stringCompare(buff, "STOP", strlen("STOP"))) {
		HAL_UART_Transmit(this->huart, (uint8_t*) "Stopped \n",
				strlen("Stopped \n"), 500);
		Mot->stop();
		///////////////
	} else if (stringCompare(buff, "FIRE", strlen("FIRE"))) {
		HAL_UART_Transmit(this->huart, (uint8_t*) "BOMBS AWAY \n",
				strlen("BOMBS AWAY \n"), 500);
...
void Bluetooth_module::doCommand1() {
	...
	if (stringCompare(buff, "TEST_MOTOR", strlen("TEST_MOTOR"))) {
		...
		HAL_UART_Transmit(this->huart, (uint8_t*) "^ wrong command received \n",	strlen("^ wrong command received \n"), 500);
	}
}
